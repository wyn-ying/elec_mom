...
def find_files(suffix):
    path = fileDir if suffix is None else fileDir + suffix
    print(f'find_files path: {path}')
    music_files = []
    for root, dirs, files in os.walk(path):
        subdir = root.replace(fileDir, '')
        if subdir.startswith('/'):
            subdir = subdir[1:]
        for file in files:
            if file.lower().split('.')[-1] in ['flac','mp3','wav','aac','m4a']:
                fname = file if not subdir else os.path.join(subdir, file)
                music_files.append(fname)
    print(music_files)
    return music_files

def updateFileListV2(request_path):
    suffix = request_path.split('_')[0]
    suffix = None if suffix == '/playlist' else suffix
...
class meHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            self.return302(fileList[0])
            fileIndex = 1
        elif self.path in ('/playlist', '/story_playlist', '/poetry_playlist'):
            playlist_request_count += 0
            if playlist_request_count <= 10:
                # updateFileList()
                updateFileListV2(self.path)
